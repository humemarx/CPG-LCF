...
class FCN2D(nn.Module):
    def __init__(self, base_block, fpn_block, base_channels, base_layers, base_strides):
        ...
        for i in range(len(self.base_layers)):
            ...
            self.base_layers[i], stride=self.base_strides[i], dilation=1))
    ...
    def _make_layer(self, block, in_planes, out_planes, num_blocks, stride=1, dilation=1):
        ...
        for i in range(num_blocks):
            layer.append(get_module(type=block, inplanes=out_planes, dilation=dilation, use_att=False))
        
        layer.append(get_module(type=block, inplanes=out_planes, dilation=dilation, use_att=True))
