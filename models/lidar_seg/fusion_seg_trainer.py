...
from ..base_trainer import gather_metric
...
class FusionSegTrainer(base_trainer.BaseTrainer):    
    def log_metric(self, epoch_metric_dics, epoch):
        fp = open(self.fpath, 'a+')
        string = '=========== epoch {0} ==========\n'.format(epoch)
        for metric_name, metric_cate in sorted(epoch_metric_dics.items(), key=lambda x:x[0]):
            if not isinstance(metric_cate, dict):
                self.writer.add_scalar("{}".format(metric_name), metric_cate, epoch)
                string = string + metric_name + ': %.4f' % (metric_cate)+'\n'
                continue

            string = string + '\n{0} \n'.format(metric_name)
            for key, value in metric_cate.items():
                self.writer.add_scalar("{}/{}".format(metric_name, key), value, epoch)
                string = string + key + ': ' + str(value)+'\n'
        fp.write(string + '\n')
        fp.close()

    def export_best_model(self):
        path_ckpt = os.path.join(self.best_path, 'bestmodel.ckpt')
        shutil.copy(self.best_ckpt, path_ckpt)


    def summary_image(self, model_image, name_prefix='', name=''):
        batch_size = model_image.shape[0]
        for ib in range(batch_size):
            model_img = torch.zeros((model_image.shape[1], model_image.shape[2], 3), dtype=torch.float32)
            for idx, colors in enumerate(color_map):
                model_img[model_image[ib,:,:,0]==idx] = torch.Tensor(colors)
            model_img = model_img/255.0
            model_img = model_img.unsqueeze(dim=0)
            model_img = model_img.permute(0, 3, 1, 2).contiguous()
            self.writer.add_images('{}/{}/{}'.format(name_prefix,ib,name), model_img, global_step=self.global_step_idx, walltime=None, dataformats='NCHW')

    def cv_image(self, model_image, name_prefix='', name=''):
        batch_size = model_image.shape[0]
        for ib in range(batch_size):
            model_img = torch.zeros((model_image.shape[1], model_image.shape[2], 3), dtype=torch.float32)
            for idx, colors in enumerate(color_map):
                model_img[model_image[ib,:,:,0]==idx] = torch.Tensor(colors)
            model_img = model_img.cpu().numpy().astype(np.uint8)
            cv2.imwrite('{}_{}_{}.jpg'.format(name_prefix,ib,name), model_img)
    ...
    def training_step(self, batch_idx, batch):
        ...
        if self.pGen.use_aux:
            bev_label = batch['bev_label']
            rv_label = batch['rv_label']

        pred_result = self.model(pcds_xyzi, pcds_coord, pcds_sphere_coord)
        pred_cls_list = pred_result['pred_cls_list']
        bev_seg_list = pred_result['bev_seg_list']
        rv_seg_list = pred_result['rv_seg_list']

        if self.pGen.use_consistency:
            pred_result_raw = self.model(pcds_xyzi_raw, pcds_coord_raw, pcds_sphere_coord_raw)
    ...
    def validation_step(self, batch_idx, batch):
        ...
        # forward
        pred_result = self.model(pcds_xyzi.cuda(), pcds_coord.cuda(), pcds_sphere_coord.cuda())
        pred_cls_list = pred_result['pred_cls_list']
    ...
    def training_epoch_end(self):
        # reset frame point nums
        is_aug = True
        self.train_dataset.reset_sample(is_aug=is_aug)
    
    def training_end(self):
        if self.global_rank == 0:
            metric_path = os.path.join(self.save_path, "metric_best.txt")
            shutil.copy(self.fpath, metric_path)
